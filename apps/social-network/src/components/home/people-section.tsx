...
export function PeopleSection({ className }: HTMLAttributes<HTMLDivElement>) {
  ...
  useEffect(() => {
    api.getUsers(getPaginationParams(1, 5)).then((response) => {
      ...
      <div className="-mx-4 flex items-center gap-3 overflow-x-auto px-4 md:mx-0 md:grid md:grid-cols-3 md:grid-rows-3 md:px-0">
        {!loading
          ...
          : Array.from(Array(5)).map((_, i) => (
              <div
                key={i}
                className="bg-accent text-primary relative flex aspect-[4/3] w-[45%] flex-none flex-col items-start justify-end gap-2 rounded-sm p-3 md:h-full md:w-full first:md:col-span-2 first:md:row-span-2 first:md:aspect-auto"
              >
